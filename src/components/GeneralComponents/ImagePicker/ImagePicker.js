...
const Component = ({onSelectItem, navigation, ...otherProps}) => {
  ...
  const cloudinaryUpload = async (photo) => {
    ...
    data.append('upload_preset', 'letsshare');
    data.append('cloud_name', 'letsshare');
    await fetch('https://api.cloudinary.com/v1_1/letsshare/image/upload', {
      method: 'post',
      body: data,
    })
      .then((res) => res.json())
